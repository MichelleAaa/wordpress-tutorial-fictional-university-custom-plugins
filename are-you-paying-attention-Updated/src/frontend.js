...
//We didn't install React or ReactDOM, but we don't need to install it. -- wp-element has an abstraction of React, which is why we can use React without an installation in package.json. We always just point towards the one copy of react.
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function Quiz(props) {
    ...
    return (
      ...
      {/* //Note that this is not normal CSS titles for the properties.  */}
        <p>{props.question}</p>
        <ul>
          {props.answers.map(function (answer, index) {
            return (
              <li className={(isCorrectDelayed === true && index == props.correctAnswer ? "no-click" : "") + (isCorrectDelayed === true && index != props.correctAnswer ? "fade-incorrect" : "")} onClick={isCorrect === true ? undefined : () => handleAnswer(index)}>
                {/* If the answer is correct, then a green checkbox SVG will display */}
                ...
                {/* If the answer they clicked on is not the correct answer, then an x SVG will display */}
                {isCorrectDelayed === true && index != props.correctAnswer && (
